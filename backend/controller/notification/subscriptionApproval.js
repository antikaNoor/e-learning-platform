    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .send(
            failure(
              "This course does not exist or is not published. Please enter a valid course."
            )
          );
      }

      // check is student is already enrolled in the course
      const student = await authModel.findOne({ _id: req.user._id }).populate({
        path: "studentID",
        select: "email",
      });
      ...
      if (existingRequest) {
        return res
          .status(400)
          .send(failure("You have already requested for subscription."));
      }
      ...
      return res
        ...
        .send(
          success(
            "Requested successfully and notification has been sent to the admin for approval."
          )
        );
    } catch (error) {
  ...
  async showAllSubscriptionRequest(req, res) {
    try {
      ...
      const notifications = await notificationModel
        .find({ type: "subscription_request" })
        .populate({
          path: "from",
      ...
      return res
        .status(200)
        .send(success("All subscription requests", notifications));
    } catch (error) {
      console.log("error", error);
      return res.status(500).send(failure("Internal server error"));
    }
    ...
    try {
      ...
      if (!existingNotification) {
        return res
          ...
          .send(
            failure(
              "This notification does not exist. Please enter a valid notification."
            )
          );
      }

      const existingCourse = await courseModel.findOne({
        _id: new mongoose.Types.ObjectId(existingNotification.courseID),
      });
      ...
      if (existingCourse.isApproved !== true) {
        return res
          .status(400)
          .send(failure("This course has not been approved by admin."));
      }

      const student = await authModel.findOne({
        _id: existingNotification.from._id,
      });
      const existingStudent = await studentModel.findOne({
        email: existingNotification.from.email,
      });
      ...
      if (action === "approve") {
        ...
        const existingWish = await wishListModel.findOne({
          studentID: new mongoose.Types.ObjectId(existingStudent._id),
        });
        if (existingWish) {
          existingWish.courseID = existingWish.courseID.filter(
            (course) => !course.equals(existingCourse._id)
          );
        ...
        if (existingCart) {
          existingCart.courseID = existingCart.courseID.filter(
            (course) => !course.equals(existingCourse._id)
          );
        ...
        const htmlBody = await ejsRenderFile(
          path.join(__dirname, "../../views/SubscriptionApprovalEmail.ejs"),
          {
        ...
        const emailResult = await sendMail(
          existingNotification.from.email,
          "Subscription Approval",
          htmlBody
        );
        ...
        return res
          ...
          .send(
            success(
              `Subscription request for ${existingCourse.title} has been approved successfully.`
            )
          );
        ...
        return res
          ...
          .send(
            success(
              `Subscription request for ${existingCourse.title} has been rejected.`
            )
          );
      }
