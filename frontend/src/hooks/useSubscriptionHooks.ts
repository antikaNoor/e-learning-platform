import {
    AddToCartApi, RemoveFromCartApi, AddToWishlistApi,
    RemoveFromWishlistApi, SubscribeApi,
    RespondToTeacherReqApi, RespondToSubscribeApi, RespondToCourseReqApi
...
const useSubscription = () => {
    ...
    const respondToCourseReq = useCallback(
        async (data: string, notificationID: string, token: string) => {
            console.log("token from hook", token);
            try {
                await RespondToCourseReqApi(data, notificationID, token);
                console.log("response from hook");
            } catch (error: any) {
                console.log(error);
            }
